...
def drive_backup_db():
    ...
    try:
        ...
        error_text = last_error or "فشل الاتصال بخدمة Google Drive."
        if "Unauthorized" in str(error_text):
            error_text = (
                "المزامنة رفضت التوكن. تأكد أن DRIVE_BACKUP_TOKEN في "
                "Streamlit Secrets مطابق تمامًا لـ BACKUP_TOKEN في Apps Script، "
                "ثم اعمل Deploy لنسخة جديدة من Web App.\n"
...
# =========================================================
    ...
    if df.empty:
        ...
        # لا ننشئ عمودًا جديدًا للمتبقي.
        # المتبقي موجود بالفعل داخل عمود "حالة الدفع"، ونلوّن نفس الخلية
        # حتى يظل الجدول كما هو بدون أعمدة إضافية.
        def payment_status_style(value):
        ...
        styled_df = df.style.apply(
            lambda col: [payment_status_style(v) for v in col],
            subset=["حالة الدفع"]
        )
